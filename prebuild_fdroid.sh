...
grep -q '\./prebuild/build_tlottie\.sh' $prep || { echo "$prep: tlottie build call moved" >&2; exit 1; }
sed -i -e "s|\./prebuild/build_tlottie\.sh|TLOTTIE_RUST_TOOLCHAIN=$rust_toolchain &|" $prep
